...
class GitProxyClient:
    ...
    def push_bundle(
        ...
        with open(bundle_path, "rb") as f:
            ...
            response = requests.post(
                f"{self.proxy_url}/git/push-bundle", files=files, data=data, headers=self._auth_headers(), timeout=600
            )
...
def setup_git_user(repo_dir: str, email: str = "claude@anthropic.com", name: str = "Claude") -> None:
    ...
    subprocess.run(["git", "config", "user.name", name], cwd=repo_dir, check=True, capture_output=True)


def clone_repo(repo_url: str, target_dir: str, branch: str = "main", setup_user: bool = True) -> str:
    """
    ...
    # Clone from bundle
    subprocess.run(["git", "clone", bundle_path, target_dir], capture_output=True, text=True, check=True)

    # Set remote URL
    subprocess.run(
        ["git", "remote", "set-url", "origin", repo_url], cwd=target_dir, capture_output=True, text=True, check=True
    )
