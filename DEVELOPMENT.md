...
**Token expiry:** In Google Cloud's testing mode, OAuth2 tokens expire after 7 days absolutely (regardless of usage). In production mode, tokens expire only after 6 months of inactivity. To promote to production for personal use: no verification or cost — just click "Publish App" in Google Cloud Console and re-authorize. If tokens expire, re-authorize with `uv run python scripts/google_oauth_setup.py --refresh <label>`.

## Infrastructure

- Wildcard DNS `*.joshuashew.com` -> Cloudflare Tunnel (dashboard-managed)
- MCP server on port 10000, Flask proxy on port 8443
- Local stdio MCP server via `.mcp.json` for service management (launchctl, logs, setup)
- Google OAuth app promoted to Production (no more 7-day token expiry)

## Troubleshooting

### Cloudflare Tunnel + AI Services

When using Cloudflare Tunnel with AI services (Claude.ai, etc.), check:
- **Block AI Bots** (Security -> Bots) — must be OFF; silently drops `python-httpx` requests
- **Bot Fight Mode** (Security -> Bots) — can also interfere
- Cloudflare Workers are unaffected (zone-level bot settings don't apply)

### MCP Server Configuration

- No custom middleware needed — default FastMCP works directly with Cloudflare Tunnel
- Must pass `lifespan=fastmcp_app.lifespan` to parent Starlette app (only if wrapping with custom middleware)
- GitHub OAuth Apps don't issue refresh tokens (only GitHub Apps do)
- GitHub OAuth callback URL must exactly match the domain (no subdomain wildcards)
- **`mcp/` directory naming collision**: The local `mcp/` directory collides with the pip `mcp` package. Can't use `python -m mcp.X`. Must run scripts directly (`python path/to/file.py`) and use `sys.path.insert` for sibling imports.
